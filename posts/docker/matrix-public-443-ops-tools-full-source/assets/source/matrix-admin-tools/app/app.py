...
def db():
    ...
    conn.commit()
    return conn
    # task progress/stop/cleanup columns, forward-compat for existing app.db
    ...
    if "stopped" not in job_columns:
        conn.execute("alter table jobs add column stopped integer not null default 0")
    if "pack_ids" not in job_columns:
        conn.execute("alter table jobs add column pack_ids text")
...
def update_job_progress(job_id, progress_text):
    conn = db()
    conn.execute("update jobs set progress=? where id=?", (progress_text, job_id))
    conn.commit()
    conn.close()
...
def request_job_stop(job_id):
    conn = db()
    conn.execute("update jobs set stopped=1 where id=?", (job_id,))
    conn.commit()
    conn.close()


def is_job_stopped(job_id):
    conn = db()
    row = conn.execute("select stopped from jobs where id=?", (job_id,)).fetchone()
    conn.close()
...
def cleanup_job(job_id):
    conn = db()
    job = conn.execute("select pack_ids, kind from jobs where id=?", (job_id,)).fetchone()
    if job and job["pack_ids"]:
        pack_ids = [int(x.strip()) for x in job["pack_ids"].split(",") if x.strip()]
        for pid in pack_ids:
            conn.execute("delete from sticker_packs where id=?", (pid,))
...
def _download_sticker_media_with_retry(mxc, max_retries=3):
    """下载贴纸媒体，带指数退避重试，兼容 Synapse 启动期短暂断连。"""
    for attempt in range(max_retries):
        try:
            return proxy_matrix_media(mxc)
        except (urllib.error.URLError, ConnectionError, OSError, BrokenPipeError) as e:
            ...
            # 非网络类异常（如媒体损坏）不重试，直接透传
            raise


def rebuild_stickerpicker_static_assets():
    """全量导出贴纸静态资源到 packs/ 目录。

    改进：
    * media 下载带指数退避重试，避免 Synapse 启动竞争导致全量跳过
    * 先写入临时目录，导出成功后再原子替换，失败则保留旧文件
    """
    packs = list_public_sticker_packs()
    # 写入临时目录，保证旧文件不被破坏
    tmp_root = STICKER_PACKS_DIR + ".tmp-" + uuid.uuid4().hex
    ...
    for item in packs:
        ...
        for sticker in stickers:
            ...
            export_target = os.path.join(
                tmp_root, sticker_pack_media_web_path(sticker, preview_mimetype)
            )
            try:
                _, payload = _download_sticker_media_with_retry(preview_mxc)
        ...
        write_json_atomic(os.path.join(tmp_root, f"{pack['slug']}.json"), pack_payload)
        total_exported += len(exported_stickers)

    write_json_atomic(os.path.join(tmp_root, "index.json"), sticker_index_payload())

    # 原子替换：只有成功导出至少一张贴纸才替换，否则保留旧文件
    if total_exported > 0:
        # Docker overlay 文件系统上 os.rename 可能报 Cross-device link，
        # 用 shutil.move 兼容跨设备移动（拷贝+删除）。
        shutil.rmtree(STICKER_PACKS_DIR, ignore_errors=True)
        shutil.move(tmp_root, STICKER_PACKS_DIR)
        append_log(f"sticker export completed: {total_exported} stickers across {len(packs)} packs")
    else:
        shutil.rmtree(tmp_root, ignore_errors=True)
        append_log("sticker export ABORTED: zero stickers exported, existing files preserved")
...
def send_notice_job(job_id, form):
    update_job_progress(job_id, "准备发送通知...")
    ...
    update_job_progress(job_id, "通知发送完成")
...
<div class="job-detail">
  ...
  <details class="wide" open><summary>查看执行摘要</summary><pre>{esc(summary_text)}</pre></details>
    ...
    if job_status == "running":
        actions.append(
            "<form method=\"post\" action=\"/jobs/" + esc(job_id) + "/stop\" style=\"display:inline\">"
            "<button class=\"danger btn-small\">停止</button></form>"
        )
    else:
        actions.append(
            "<form method=\"post\" action=\"/jobs/" + esc(job_id) + "/delete\" style=\"display:inline\""
            " onsubmit=\"return confirm('确认删除这条任务记录？')\">"
            "<button class=\"secondary btn-small\">删除</button></form>"
        )
    actions_html = " ".join(actions) if actions else ""
    detail = "<details><summary>查看详情</summary><pre>" + esc(output) + "</pre></details>"
    if actions_html:
        return "<div class=\"job-actions\">" + actions_html + "</div>" + detail
    return detail
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .sidebar-nav button .job-badge {{
      display:none;
      vertical-align:middle; margin-left:6px;
      background:#dc2626; color:#fff; font-size:11px; font-weight:600;
      padding:1px 7px; border-radius:10px; line-height:18px;
    }}
    .sidebar-nav button .job-badge.show {{ display:inline-block; }}
    .job-progress {{ display:inline-block; font-size:12px; color:var(--muted); }}
    .btn-small {{ font-size:12px; padding:3px 10px; margin:2px; border-radius:6px; }}
    .job-actions {{ margin-bottom:6px; display:flex; gap:6px; align-items:center; flex-wrap:wrap; }}
    .job-action-cell {{ min-width:200px; }}
    .job-detail pre {{ background:#f1f5f9; color:#1e293b; padding:10px; border-radius:6px; font-size:12px; max-height:300px; overflow:auto; white-space:pre-wrap; }}
    .job-detail code {{ background:#f1f5f9; padding:1px 4px; border-radius:3px; font-size:12px; }}
    .badge.warn {{ background:#f59e0b; color:#fff; }}
    .job-toolbar {{ display:flex; align-items:center; gap:12px; margin-bottom:10px; flex-wrap:wrap; }}
    .job-filters {{ display:flex; gap:4px; }}
    .filter-btn {{ font-size:12px; padding:4px 12px; border-radius:6px; border:1px solid #d1d5db; background:transparent; color:var(--text); cursor:pointer; }}
    .filter-btn.active {{ background:#eef4ff; border-color:#bfdbfe; font-weight:600; }}
...
<div class="app-shell">
  <aside class="app-sidebar">
    ...
    <nav class="sidebar-nav" aria-label="运维面板分类">
      ...
      <button type="button" class="panel-tab" data-panel="jobs"><strong>最近任务 <span id="job-badge" class="job-badge">0</span></strong><span>查看最近执行结果和错误摘要</span></button>
    </nav>
  </aside>
...
(() => {{
  ...
  const updateJobBadge = () => {{
    fetch("/jobs/running-count", {{ cache: "no-store" }})
      .then((r) => r.text())
      .then((count) => {{
        const n = parseInt(count, 10) || 0;
        if (n > 0) {{
          jobBadge.textContent = n;
  ...
  // 刷新进度，任务完成时原地更新状态不刷新页面
  const updateJobProgress = () => {{
    document.querySelectorAll(".job-progress[data-job-id]").forEach((el) => {{
      // 跳过已处理过的（防止重复轮询同个任务)
      if (el.dataset.procDone) return;
      const jobId = el.dataset.jobId;
      fetch("/jobs/" + encodeURIComponent(jobId) + "/progress", {{ cache: "no-store" }})
        .then((r) => r.json())
        .then((data) => {{
          ...
          // 标记已处理，后续轮询跳过
          el.dataset.procDone = "1";
          // 任务已完成/失败/取消，原地更新，不刷新页面
          ...
          if (badgeSpan) {{
            let cls = "ok";
            if (data.status === "failed") cls = "bad";
            else if (data.status === "cancelled") cls = "warn";
          ...
          if (tds.length >= 5) tds[4].textContent = data.summary || "";
          // 移除停止按钮，同时更新详情区域
          const stopForm = row.querySelector("form[action*='/stop']");
          if (stopForm) stopForm.remove();
          // 从服务端重新获取详情 HTML（根据 job 状态渲染正确的详情格式）
          const actionCell = row.querySelector(".job-action-cell");
          if (actionCell) {{
            fetch("/jobs/" + encodeURIComponent(jobId) + "/detail", {{ cache: "no-store" }})
              .then((r) => r.text())
              .then((html) => {{ if (html) actionCell.innerHTML = html; }})
              .catch(() => {{}});
          }}
        }})
        .catch(() => {{}});
  ...
  if (jobBadge) {{
    updateJobBadge();
  // 页面加载后立即检查一次任务状态，不等 3 秒轮询
  setTimeout(updateJobProgress, 500);
    setInterval(updateJobBadge, 5000);
  }}
  updateJobProgress();
  setInterval(updateJobProgress, 3000);

  // select all toggle
  const selectAll = document.getElementById("select-all-jobs");
  if (selectAll) {{
    selectAll.addEventListener("change", () => {{
      document.querySelectorAll("#jobs tbody input[name='job_ids']").forEach((cb) => {{
    ...
    btn.addEventListener("click", () => {{
      ...
      document.querySelectorAll("#jobs tbody tr").forEach((tr) => {{
        if (filter === "all") {{
          ...
          tr.style.display = kind.includes(filter) ? "" : "none";
        }}
      }});
    }});
  }});
...
def render_home(message=""):
    ...
    for row in jobs:
        ...
        if row["status"] == "running":
            progress_cell = "<span class=\"job-progress\" data-job-id=\"" + esc(row["id"]) + "\">" + esc(row["progress"] or "") + "</span>"
        elif row["status"] in ("failed", "cancelled") and row["summary"]:
            progress_cell = "<span class=\"muted\" style=\"font-size:12px\">" + esc(short_text(row["summary"], 60)) + "</span>"
        rows.append(
            "<tr data-kind=\"" + esc(row["kind"]) + "\">"
            "<td><input type=\"checkbox\" name=\"job_ids\" value=\"" + esc(row["id"]) + "\" form=\"batch-delete-form\"></td>"
            "<td>" + esc(row["created_at"]) + "<br><span class=\"muted\">" + esc(finished) + "</span></td>"
            "<td>" + esc(row["kind"]) + "</td>"
            "<td><span class=\"badge " + status_class(row["status"]) + "\">" + esc(row["status"]) + "</span><br>" + progress_cell + "</td>"
            "<td>" + esc(row["summary"] or "") + "</td>"
            "<td class=\"job-action-cell\">" + render_job_detail(row) + "</td></tr>"
        )
    body = f"""
...
<section id="jobs" class="app-panel" data-panel="jobs">
  <h2>最近任务 <span class="muted" style="font-weight:400;font-size:13px">（点击停止可中断运行中的任务，停止时自动清理残留）</span></h2>
  <div class="job-toolbar">
    <div class="job-filters" id="job-filters">
      <button type="button" class="filter-btn active" data-filter="all">全部</button>
      <button type="button" class="filter-btn" data-filter="telegram-import">贴纸导入</button>
      <button type="button" class="filter-btn" data-filter="server-notice">发送通知</button>
      <button type="button" class="filter-btn" data-filter="media-purge">媒体清理</button>
    </div>
    <button type="submit" class="secondary btn-small" form="batch-delete-form" onclick="return confirm('确认删除选中的任务记录？')">删除选中</button>
  </div>
  <form id="batch-delete-form" method="post" action="/jobs/batch-delete"></form>
  <table><thead><tr><th><input type="checkbox" id="select-all-jobs"></th><th>创建/完成时间</th><th>类型</th><th>状态/进度</th><th>摘要</th><th>操作</th></tr></thead><tbody>{''.join(rows)}</tbody></table>
</section>
"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/stickers":
            ...
            return
        if path == "/jobs/running-count":
            self.send_bytes(str(running_job_count()).encode("utf-8"), "text/plain")
            return
        if path.startswith("/jobs/") and path.endswith("/progress"):
            job_id = path.split("/")[2]
            conn = db()
            row = conn.execute("select progress, status, summary, finished_at, kind from jobs where id=?", (job_id,)).fetchone()
            conn.close()
            if not row:
                self.send_response(404)
                self.end_headers()
        ...
        if path != "/":
            ...
            self.end_headers()
            return
        # parse redirect message (PRG pattern)
        _query = urllib.parse.urlsplit(self.path).query
        _msg = urllib.parse.parse_qs(_query).get("msg", [None])[0]
        self.send_html(render_home(_msg or ""))
    def do_POST(self):
        if not self.require_auth():
            return
        path = self.path_only()
        ...
        try:
            ...
            if path == "/notice":
                ...
                self.send_header("Location", "/?msg=" + urllib.parse.quote("已启动通知任务 " + job_id))
                self.end_headers()
                return
            if path == "/jobs/batch-delete":
                ids = form.get("job_ids", [])
                if ids:
                    conn = db()
                    placeholders = ",".join("?" for _ in ids)
                    conn.execute("delete from jobs where id in (" + placeholders + ")", ids)
                    conn.commit()
                    conn.close()
                self.send_html(render_home("Deleted " + str(len(ids)) + " job(s)"))
                return
            if path.startswith("/jobs/") and path.endswith("/delete"):
                ...
                conn.commit()
                conn.close()
                self.send_html(render_home("Deleted job " + str(job_id)))
                return
            if path == "/stickers":
    ...
    def handle_stickers(self, form, files):
        ...
        if action == "import_telegram_pack":
            ...
            _conn.close()
            if _exists:
                _msg = "该 Telegram 贴纸包已导入过：#" + str(int(_exists[0])) + " " + str(_exists[1])
                self.send_response(303)
                self.send_header("Location", "/?msg=" + urllib.parse.quote(_msg))
                self.end_headers()
                return
            job_id = create_job("telegram-import")
            threading.Thread(target=self.import_telegram_thread, args=(job_id, form), daemon=True).start()
            self.send_response(303)
            self.send_header("Location", "/?msg=" + urllib.parse.quote("已启动 Telegram 贴纸包导入任务 " + job_id))
            self.end_headers()
            return
    ...
    def import_telegram_thread(self, job_id, form):
        try:
            update_job_progress(job_id, "Fetching pack info from Telegram...")
            short_name = normalize_telegram_pack_ref(form.get("telegram_pack_ref", [""])[0])
            result = telegram_api_call("getStickerSet", name=short_name)
            stickers = result.get("stickers") or []
            if not stickers:
                raise ValueError("This Telegram sticker pack has no importable stickers")
            unsupported = [item.get("type") for item in stickers if item.get("is_animated")]
            if unsupported:
                raise ValueError("TGS/Lottie animated stickers are not supported yet")
            author_name = form.get("authorName", [""])[0].strip() or "Telegram"
            values = {
                "title": form.get("name", [""])[0].strip() or result.get("title") or short_name,
                "description": form.get("description", [""])[0].strip() or result.get("title") or short_name,
                ...
                "is_public": 1 if form.get("isPublic", ["1"])[0] == "1" else 0,
                "author_name": author_name,
                "author_reference": form.get("authorReference", [""])[0].strip() or "https://t.me/addstickers/" + short_name,
                "license": form.get("license", ["Telegram Imported"])[0].strip() or "Telegram Imported",
            }
            conn = db()
            exists = conn.execute(
            ...
            if exists:
                conn.close()
                raise ValueError("This Telegram pack was already imported: #" + str(int(exists["id"])) + " " + exists["title"])
            pack_id = create_sticker_pack_record(conn, values, source_type="telegram", source_ref=short_name)
            conn.execute("update jobs set pack_ids=? where id=?", (str(pack_id), job_id))
            conn.commit()
            conn.close()
            imported = 0
            skipped = 0
            ...
            for index, item in enumerate(stickers, 1):
                ...
                update_job_progress(job_id, "Downloading sticker " + str(index) + "/" + str(total) + "...")
                file_id = item.get("file_id")
                unique_ref = item.get("file_unique_id") or file_id
                if not file_id or not unique_ref:
                    skipped += 1
                ...
                if content_type:
                    mimetype = content_type.split(";", 1)[0].strip()
                mimetype = detect_binary_mimetype(original_name, binary, mimetype)
                update_job_progress(job_id, "Uploading sticker " + str(index) + "/" + str(total) + " to Matrix...")
                image_mxc = upload_matrix_media(original_name, binary, mimetype)
                preview_image_mxc = None
                preview_image_mimetype = None
                send_image_mxc = None
                ...
                if width <= 0 or height <= 0:
                    width, height = detect_image_size(binary, mimetype)
                name = item.get("emoji") or item.get("set_name") or "telegram-" + str(index)
                body = item.get("emoji") or name
                conn = db()
                insert_sticker_record(
                    conn=conn, pack_id=pack_id, name=name, body=body,
                    image_mxc=image_mxc, preview_image_mxc=preview_image_mxc,
                    preview_image_mimetype=preview_image_mimetype,
                    send_image_mxc=send_image_mxc, send_image_mimetype=send_image_mimetype,
                    source_ref=unique_ref, mimetype=mimetype,
                    width=width, height=height, size_bytes=len(binary),
                )
                conn.commit()
                conn.close()
                imported += 1
            if imported == 0:
                raise ValueError("No stickers were successfully imported from this pack")
            conn = db()
            refresh_pack_avatar(conn, pack_id)
            conn.commit()
            conn.close()
            update_job_progress(job_id, "Exporting static assets...")
            rebuild_stickerpicker_static_assets()
            update_job_progress(job_id, "Syncing sticker widgets...")
            sync_sticker_widgets(restart_synapse=False)
            result_data = {"pack_id": pack_id, "imported": imported, "skipped": skipped, "short_name": short_name}
            finish_job(
                job_id,
                "completed",
                "Imported " + str(imported) + ", skipped " + str(skipped) + ", pack #" + str(pack_id),
                json.dumps(result_data, ensure_ascii=False),
            )
        except InterruptedError:
            # 停止时自动清理残留：删除关联的贴纸包（级联删除贴纸），保留任务记录供查看
            _conn = db()
            _job = _conn.execute("select pack_ids from jobs where id=?", (job_id,)).fetchone()
            if _job and _job["pack_ids"]:
                for _pid in [int(x.strip()) for x in _job["pack_ids"].split(",") if x.strip()]:
                    _conn.execute("delete from sticker_packs where id=?", (_pid,))
            _conn.commit()
            _conn.close()
            finish_job(job_id, "cancelled", "User cancelled import, residual cleaned", "")
        except Exception as e:
            finish_job(job_id, "failed", "Telegram import failed: " + str(e), str(e))
