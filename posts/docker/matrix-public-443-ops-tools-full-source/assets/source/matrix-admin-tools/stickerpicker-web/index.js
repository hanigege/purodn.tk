...
const CACHE_VERSION = params.get("v") || ""
...
const makeThumbnailURL = sticker => sticker.preview_url || `${PACKS_BASE_URL}/thumbnails/${sticker.url.split("/").slice(-1)[0]}`
const isVideoSticker = sticker => (sticker.preview_mimetype || sticker.info?.mimetype || "").startsWith("video/")
...
// 检测是否独立窗口打开（Element 点"编辑"时在新标签页打开 URL，不在 iframe 中）
...
class App extends Component {
	...
	_loadPacks(disableCache = false) {
		...
		fetch(withVersion(INDEX), {cache}).then(async indexRes => {
			...
			for (const packFile of indexData.packs) {
				...
				if (packFile.startsWith("https://") || packFile.startsWith("http://")) {
					packRes = await fetch(withVersion(packFile), {cache})
				} else {
					packRes = await fetch(withVersion(`${PACKS_BASE_URL}/${packFile}`), {cache})
				}
	...
	observeImageIntersections(intersections) {
		for (const entry of intersections) {
			const media = entry.target.children.item(0)
			if (!media) {
				continue
			}
	...
	render() {
		...
		// 独立窗口打开时（如 Element 的"编辑"按钮），显示友好提示
		if (isStandalone) {
			return html`
				<main class="standalone ${theme}">
					<div class="standalone-card">
						<h1>贴纸包选择器</h1>
						<p>这是 Matrix 贴纸包选择器页面。</p>
						<p>请回到 Element 聊天窗口，在贴纸面板中使用此选择器发送贴纸。</p>
						<hr style="border:none;border-top:1px solid var(--highlight-color,#ddd);margin:1rem 0"/>
						<p style="font-size:0.85rem;color:var(--text-secondary,#888)">
							当前贴纸包数量：${this.state.packs.length}
						</p>
					</div>
				</main>
			`
...
const renderPackNavMedia = pack => {
	const coverSticker = pack?.stickers?.[0]
	if (!coverSticker) {
		return null
	}
	// 视频贴纸包的顶部入口图标不能再固定按图片渲染，否则会出现包内正常、入口破图的问题。
	if (isVideoSticker(coverSticker)) {
		return html`
			<video src=${makeThumbnailURL(coverSticker)} muted loop autoplay playsinline class="visible" title=${coverSticker.body}></video>
		`
	}
	return html`
		<img src=${makeThumbnailURL(coverSticker)} alt=${coverSticker.body} class="visible" />
	`
}
...
const Sticker = ({content, send}) => html`
	<div class="sticker" onClick=${send} data-sticker-id=${content.id}>
		${isVideoSticker(content)
			? html`<video data-src=${makeThumbnailURL(content)} muted loop playsinline title=${content.body}></video>`
			: html`<img data-src=${makeThumbnailURL(content)} alt=${content.body} title=${content.body}/>`
		}
	</div>
`
